...
{
  if (!cnn_type.compare("yolo") || !cnn_type.compare("mobilenetssd"))
  {
    ...
    for (int j = 0; j < grid_width; j++)
    {
      for (int k = 0; k < bbox_num; k++)
      {
        ...
        if (obj_in_bbox.item.probability > prob_threshold)
        {
          ...
          obj_in_bbox.bbox.width = (result[prob_num + bbox_conf_num + index * 4 + 2]) * (result[prob_num + bbox_conf_num + index * 4 + 2]) * img_width;
          obj_in_bbox.bbox.height = (result[prob_num + bbox_conf_num + index * 4 + 3]) * (result[prob_num + bbox_conf_num + index * 4 + 3]) * img_height;
          obj_in_bbox.bbox.x = (x_center - 0.5 * obj_in_bbox.bbox.width) < 0 ? 0 : (x_center - 0.5 * obj_in_bbox.bbox.width);
          obj_in_bbox.bbox.y = (y_center - 0.5 * obj_in_bbox.bbox.height) < 0 ? 0 : (y_center - 0.5 * obj_in_bbox.bbox.height);
                            ...
                            int img_width, int img_height)
{
  ...
  for (int i = 0; i < num_detection; i++)
  {
    ...
    ItemInBBox obj_in_bbox;
    if (std::isnan(category_id) || std::isnan(probability) || std::isnan(xmin) || xmin < 0 || xmin > img_width || std::isnan(ymin) || ymin < 0 || ymin > img_height || std::isnan(xmax) || xmax < 0 || xmax > img_width || std::isnan(ymax) || ymax < 0 || ymax > img_height|| probability<0.55)
    {
      continue;
    }
    else
...
{
  int xmax = (box1.bbox.x + box1.bbox.width < box2.bbox.x + box2.bbox.width) ? box1.bbox.x + box1.bbox.width : box2.bbox.x + box2.bbox.width;
  int xmin = (box1.bbox.x > box2.bbox.x) ? box1.bbox.x : box2.bbox.x;
  int ymax = (box1.bbox.y + box1.bbox.height < box2.bbox.y + box2.bbox.height) ? box1.bbox.y + box1.bbox.height : box2.bbox.y + box2.bbox.height;
