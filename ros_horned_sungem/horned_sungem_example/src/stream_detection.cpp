...
void syncCb(const sensor_msgs::ImageConstPtr &img, const object_msgs::ObjectsInBoxes::ConstPtr &objs_in_boxes)
{
  ...
  // ROS_INFO_STREAM("getimage width = " << width);
  // ROS_INFO_STREAM("getimage height = " << height);
  for (auto obj : objs_in_boxes->objects_vector)
  {
    ...
    cv::rectangle(cvImage, left_top, right_bottom, cv::Scalar(255, 0, 0), 1, cv::LINE_8, 0);
    cv::rectangle(cvImage, cvPoint(xmin, ymin), cvPoint(xmax, ymin + 20), cv::Scalar(255, 0, 0), -1);
...
{
  ros::init(argc, argv, "horned_sungem_example_stream");
  ros::NodeHandle nh;
  message_filters::Subscriber<sensor_msgs::Image> camSub(nh, "/hs/camera/image_raw", 1);
  message_filters::Subscriber<object_msgs::ObjectsInBoxes> objSub(nh, "/horned_sungem_nodelet/detected_objects", 1);
