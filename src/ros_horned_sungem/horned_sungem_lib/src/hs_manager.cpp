...
namespace horned_sungem_lib {
    HSManager::HSManager(const std::shared_ptr <Device> &device,
    ...
    void HSManager::initDeviceManager() {
        ...
        for (int device_index = start_device_index_; device_index < device_count_; device_index++) {
            auto hs_handle = std::make_shared<horned_sungem_lib::HS>(device_,device_index,
                                                                     static_cast<Device::LogLevel>(log_level_), camera_,
        ...
        while (1) {
            if (!image_list_.empty()) {
                ...
                if (!cnn_type_.compare("alexnet") || !cnn_type_.compare("googlenet") ||
                    !cnn_type_.compare("inception_v1") ||
                    !cnn_type_.compare("inception_v2") || !cnn_type_.compare("inception_v3") ||
                    !cnn_type_.compare("inception_v4") || !cnn_type_.compare("mobilenet") ||
