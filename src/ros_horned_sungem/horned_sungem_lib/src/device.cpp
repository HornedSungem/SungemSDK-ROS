...
{
  int level = log_level;
  int ret = hsSetGlobalOption(HS_LOG_LEVEL,
...
Device::LogLevel Device::getLogLevel()
{
  ...
  int ret = hsGetGlobalOption(HS_LOG_LEVEL,
...
Device::ThermalThrottlingLevel Device::getThermalThrottlingLevel() const
{
  ...
  int ret = hsGetDeviceOption(handle_,
                              HS_THERMAL_THROTTLING_LEVEL,
...
cv::Mat Device::getDeviceImage(bool truthy)
{
  ...
  cv::Mat mat;
  int ret = hsDeviceGetImage(getHandle(), reinterpret_cast<void **>(&device_image_data),
                             truthy);
  ExceptionUtil::tryToThrowHsException(ret);
  if (truthy)
  {
    std::vector<uint8_t> result_vector(reinterpret_cast<uint8_t *>(device_image_data),
